...
{
	int cnt = 0;     // count history record	
...
//*****************************************************************************
void microrl_init (microrl_t * pThis, void (*print) (const char *), char * prmpt, int prmpt_len) 
{
	...
	pThis->sigint = NULL;
#endif
	pThis->prompt_str = prmpt;
	pThis->prompt_len = prmpt_len;
	pThis->print = print;
...
void microrl_setprompt (microrl_t * pThis, char * prmpt, int prmpt_len) 
{
	pThis->prompt_str = prmpt;
	pThis->prompt_len = prmpt_len;
}
...
static int common_len (char ** arr)
{
	int j;
	char *shortest = arr[0];
	unsigned i, shortlen = strlen(shortest);
...
static void microrl_get_complite (microrl_t * pThis) 
{
	...
	if (compl_token[0] != NULL) {
		...
		if (len) {
			...
			if (compl_token[1] == NULL) 
				microrl_insert_text (pThis, (char*)" ", 1);
		}
		terminal_reset_cursor (pThis);
		terminal_print_line (pThis, 0, pThis->cursor);
	} 
