...
uint16_t HTU21D::readMeasurment(uint8_t cmd, uint16_t dly)
{
    ...
    err =i2c_write(my_handle, HTU21D_SAD, &cmd, 1, I2C_STOP);
    usleep(dly);
    err+=i2c_read(my_handle, HTU21D_SAD, value_read, 3);
...
{
    err=writeRegister(SOFT_RESET, 0, 0);
    usleep(15000);  //takes upto 15ms for reset
    return err;
}

uint8_t HTU21D::getOM(void)
{
    i2c_handle_t my_handle = 0;
    ...
    i2c_bus_init(I2C_BUS_I, &my_handle);
    err=i2c_write(my_handle, HTU21D_SAD, &cmd, 1, I2C_STOP);
    err=i2c_read(my_handle,HTU21D_SAD,&val,1);
    i2c_bus_deinit(&my_handle);
    return val;
}
...
float HTU21D::readHumidity(uint8_t trigger_mode)
{
    ...
    int i = (OpMode&0x01? 1:0) + (OpMode&0x80? 2:0);
